# 🧠 ÍNDICE DO SEGUNDO CÉREBRO

> Busca rápida: Use Ctrl+F para encontrar qualquer coisa

## 📅 Últimas Atualizações

### 2026-02-15 - Integração Google Sheets + Telegram
- ✅ Telegram conectado e funcionando
- ✅ GitHub configurado com token
- ✅ Google Sheets integrado (Controle Financeiro)
- ✅ Planilha financeira organizada e corrigida
- ✅ Sistema de registro automático de receitas/despesas
- 📊 Resumo: R$ 2.659,00 receitas | R$ 820,24 despesas | Saldo: R$ 1.838,76

### 2026-02-10 - Grande Sessão de Automações
- ✅ Deploy 60maisPlay no Railway
- ✅ Sistema Brain Backup criado
- ✅ 20 ideias de automação surreal listadas
- [Ver detalhes](reunioes/2026-02-10-sessao-automacoes.md)

---

## 🎯 DECISÕES IMPORTANTES

### Negócio 60maisPlay
- **Decisão:** Usar Railway para testes, MySQL na Hostgator para produção
- **Data:** 2026-02-10
- **Motivo:** Railway gratuito para validar, Hostgator para escalar
- [Ver mais](decisoes/)

### Formato de Arquivos
- **Decisão:** Sempre usar DOCX (Word), nunca Markdown
- **Data:** 2026-02-10
- **Quem:** Luis
- **Motivo:** Facilidade de edição no celular

---

## 💡 APRENDIZADOS

### Tecnologia
- Railway não roda GitHub Pages direto (precisa de deploy)
- JSON funciona bem para testes, mas MySQL é necessário para produção
- WhatsApp Business API tem limitações

### Negócio
- Plataforma de cursos precisa de acessibilidade (fonte, contraste)
- Admin é essencial para gerenciamento
- Pagamento automático aumenta conversão

---

## ✨ IDEIAS GUARDADAS

### Prioridade 1 - Próxima Sessão
1. [ ] Barra de acessibilidade 60maisPlay
2. [ ] Página de admin completa
3. [ ] MySQL na Hostgator

### Prioridade 2 - Futuro
4. [ ] Sistema de pagamento automático
5. [ ] YouTube → Newsletter → Blog automático
6. [ ] Suporte 24/7 com IA
7. [ ] Máquina de cursos automática
8. [ ] Webinars eternos

---

## 🚀 PROJETOS ATIVOS

### 60maisPlay Plataforma
- **Status:** Online (Railway)
- **URL:** https://web-production-df2db.up.railway.app
- **GitHub:** benfranklinassistente-code/60maisplay-plataforma
- **Próximo passo:** Correções de acessibilidade + Admin

### Automações Surreais (PM2)
- **#16 Assistente Pessoal 24/7** ✅ Rodando
  - 06:00: Bom dia + agenda
  - 08:00: Briefing do nicho 60+
  - 12:00: Hora do almoço
  - 18:00: Fim do expediente
  - 20:00: Preparação amanhã

- **#4 Lead Magnet Infinito** ✅ Rodando
  - Segundas 09:00: Gera novo lead magnet automaticamente

- **#19 Clonagem de Tarefas** ✅ Rodando
  - A cada 6h: Detecta padrões
  - 21:00: Relatório diário

### Interstellar OpenClaud
- **Status:** Funcionando
- **GitHub:** benfranklinassistente-code/interstellar-openclaud
- **Uso:** Gerenciamento de tarefas

---

## 🔐 ACESSOS IMPORTANTES

Ver arquivo: [config/acessos.md](config/acessos.md)

### Google Sheets
- **Planilha:** Controle Financeiro - Receitas e Despesas
- **Link:** [Abrir planilha](https://docs.google.com/spreadsheets/d/1VhY95rXzg9UjVnjr21nuOVpxhEuM3N8HN1TeeFrX7X8/edit)

### GitHub
- **Usuário:** benfranklinassistente-code
- **Token:** Salvo em variável GH_TOKEN

### Telegram
- **Bot:** Configurado e funcionando
- **User ID Luís:** 1007517562

---

## 🔍 TAGS RÁPIDAS

#acessibilidade #admin #mysql #pagamento #railway #hostgator #60maisplay #newsletter #youtube #automacao #deploy #frontend #backend

---

*Última atualização: 10/02/2026 20:20 UTC*
